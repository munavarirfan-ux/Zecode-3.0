...
export function LiveAssessmentReportHero({
  assessment,
  session,
  initials,
  onClose,
  ...
  const linkedinHref = session.linkedin
    ? session.linkedin.startsWith("http")
      ? session.linkedin
      : `https://${session.linkedin}`
    : null;
      ...
      <div className={cn(hiringHeroReportStripRow, "relative")}>
        <div className="flex min-w-0 flex-1 gap-3 sm:gap-4">
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className={hiringHeroStripTitle}>{session.name}</h2>
              <span className="inline-flex items-center gap-1 rounded-full border border-emerald-400/25 bg-emerald-500/15 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.06em] text-emerald-100">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 lg:justify-end">
          <Button
            ...
            onClick={() => toast.message("Share live report (demo)")}
          >
          ...
          <Button
            ...
            onClick={() => toast.message("Export live report (demo)")}
          >
