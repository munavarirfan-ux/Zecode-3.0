...
export function AssessmentDetailHero({
  ...
  return (
    <HiringHeroWorkspace
      aria-label="Assessment workspace header"
      heroCollapseStorageKey="assessment-detail"
      defaultHeroCollapsed
      collapsedMeta={[assessment.role, assessment.createdBy, `Created ${assessment.createdOn}`]}
      backHref={ROUTES.assessments}
      backLabel={
        <>
      ...
      actions={
        <>
          <HeroActionButton variant="secondary" onClick={onExport}>
            <Download className="h-3.5 w-3.5" strokeWidth={1.75} />
            Export
          </HeroActionButton>
          <HeroActionButton variant="secondary" onClick={onBulkInvite}>
            <Upload className="h-3.5 w-3.5" strokeWidth={1.75} />
            Bulk upload
          </HeroActionButton>
          <HeroActionButton variant="primary" onClick={onInvite}>
          ...
          <DropdownMenu modal={false}>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className={cn(hiringHeroCollapsedIconBtn, "h-9 w-9 rounded-[11px] p-0", hiringTransition)}
      ...
      metricsStorageKey={ASSESSMENT_DETAIL_METRICS_COLLAPSED_KEY}
      metrics={
