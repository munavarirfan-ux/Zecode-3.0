...
import {
  getStaticKpiTimeframeStats,
  KPI_TIMEFRAME_OPTIONS,
  persistKpiTimeframe,
  readStoredKpiTimeframe,
  type InterviewTimeframe,
} from "@/lib/dashboard/interviewTimeframeKpi";
import { cn } from "@/lib/utils";

type TimeframeHeroKpiProps = {
  kpiId: string;
  label: string;
  icon: LucideIcon;
};

export function TimeframeHeroKpi({ kpiId, label, icon: Icon }: TimeframeHeroKpiProps) {
  const [timeframe, setTimeframe] = useState<InterviewTimeframe>("today");
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setTimeframe(readStoredKpiTimeframe(kpiId));
    setMounted(true);
  }, [kpiId]);

  const stats = getStaticKpiTimeframeStats(kpiId);
  const active = stats[timeframe];
  const activeLabel = KPI_TIMEFRAME_OPTIONS.find((o) => o.value === timeframe)?.label ?? "Today";

  const onTimeframeChange = (value: string) => {
    const next = value as InterviewTimeframe;
    setTimeframe(next);
    persistKpiTimeframe(kpiId, next);
  };
  ...
  return (
    <li className={cn(hiringHeroGlassKpi, "min-w-0")}>
      ...
      <div className="relative flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[12px] border border-white/[0.16] bg-white/[0.1] shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">
            <Icon className="h-[18px] w-[18px] text-white/90" strokeWidth={1.5} aria-hidden />
          </div>
          <p className="text-[12px] font-semibold tracking-[-0.01em] text-white/90">{label}</p>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[8.5rem]">
            <DropdownMenuRadioGroup value={timeframe} onValueChange={onTimeframeChange}>
              {KPI_TIMEFRAME_OPTIONS.map(({ value, label: optLabel }) => (
      ...
      <p
        className="relative mt-3 text-[11px] leading-snug text-white/55"
        suppressHydrationWarning
      >
